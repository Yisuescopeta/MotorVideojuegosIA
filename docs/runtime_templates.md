...
El runtime exportado es el entrypoint del juego final. No incluye paneles,
inspector, tooling IA, tests ni documentacion, pero comparte el mismo
`Game` + `RuntimeController` que usa PLAY en el editor.
...
`engine/runtime/exported_game.py` crea un `SharedGameRuntime`:

1. Carga `runtime_config.json`, `game.manifest.json` y `game.pak`/`content/`.
2. Configura scripts extraidos del pack y assets via `ContentLoader`.
3. Construye `Game(editor_enabled=False, hot_reload_enabled=False)`.
4. Carga escenas con `RuntimeController.load_scene_from_data(...)`.
5. Ejecuta frames con la misma secuencia runtime que PLAY del editor.

`ExportRuntime` sigue existiendo solo como shim deprecated para no romper
imports externos; delega en `SharedGameRuntime`.
...
`--print-runtime-info` emite por stdout datos de arranque: motor, cwd, entry
scene, modo headless/smoke, proyecto y version. No ejecuta frames.
...
- Cuando `android_python_runtime` esta activo, `runtime_config.json` incluye
  `android_runtime_cache_key`, derivado del content pack y del runtime/template
  Android relevante. El shell Android extrae solo `runtime_config.json`,
  `game.manifest.json`, `levels/`, `assets/` y `scripts/` a `filesDir`, excluye
  `chaquopy/`, y reutiliza la copia si el cache key no cambia.
...
- El render Android usa la metadata `*.meta.json` empaquetada para resolver
  `Sprite.source_slice` y `Animator.slice_names`; tambien respeta `flip_x`,
  `flip_y` y la coexistencia `Sprite` + `Animator` en la misma entidad.
...
1. Carga entry scene.
2. Ejecuta 60 frames headless via `Game` + `RuntimeController`.
3. Verifica integridad del content pack (SHA-256).
4. Emite eventos runtime disponibles en esa escena.
...
> - Cobertura exhaustiva de todos los scripts de gameplay del proyecto
>
...
| `TestExportRuntimePackedScripts` | ScriptBehaviour importa scripts extraidos desde `.pak` |
| `TestExportRuntimeSystemParity` | Export no corre sistemas extra ausentes del editor para destruir pickups sin script |
| `TestExportRuntimeScriptedPickups` | Pickup scriptado aplica score y destruye entidad en export |
| `TestPrueva1ExportParity` | Prueva1 aplica score/speed/jump/life desde proyecto y pack |
...
El runtime exportado monta sistemas de juego via `SharedGameRuntime` sobre
`Game` y `RuntimeController`; las herramientas de editor quedan desactivadas.
`ExportRuntime` (`engine/runtime/export_runtime.py`) es solo shim deprecated.
Los sistemas runtime salen de la misma factory interna que usa `main.py` para
PLAY del editor. Incluyen:
...
| `AudioSystem` | Superficie ECS/runtime de audio compatible |
...
| `TimerSystem`, `TweenSystem`, `VisibleOnScreenSystem`, `ParallaxSystem`, `ResourcePreloaderSystem`, `Light2DSystem`, `ParticleSystem` | Sistemas runtime compartidos con PLAY del editor |
...
data). Los scripts ejecutan `ScriptBehaviourSystem` real con importlib fallback
sin hot-reload manager del editor.
