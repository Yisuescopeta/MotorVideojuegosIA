...
- Gameplay semantico 2D: `Collectible2D`, `Hazard2D`, `Goal2D`, `RespawnPoint2D`, `MovingPlatform2D`, `EnemyPatrol2D`, `Checkpoint2D`, `KillZone2D`, `LevelBounds2D`. Son componentes serializables. En runtime, `Gameplay2DSemanticSystem` consume contactos fisicos existentes para emitir eventos de coleccionable, hazard y goal, aplicar respawn runtime, mover `MovingPlatform2D` por su path y emitir eventos de plataforma sin modificar la escena serializada. `MovingPlatform2D` moves the platform entity, but does not yet carry riders.
