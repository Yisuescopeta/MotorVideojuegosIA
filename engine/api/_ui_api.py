...
class UIAPI(EngineAPIComponent):
    ...
    def list_ui_nodes(self) -> list[EntityData]:
        ...
        for entity in runtime.world.iter_all_entities():
            if any(entity.has_component(component) for component in (Canvas, RectTransform, UIText, UIButton, UIImage, UIPanel, UIScrollContainer, LineEdit, Slider, ProgressBar, CheckBox, SpinBox, Label, TextEdit)):
    ...
    def create_ui_lineedit(
        ...
        """Create a LineEdit UI control for single-line text input.
        ...
        Args:
            ...
            secret: If True, display characters as asterisks.
            editable: Whether the control accepts input.
            font_size: Font size in pixels.
            rect_transform: Optional RectTransform overrides.

        Returns:
            ActionResult confirming the LineEdit entity was created.
        """
        self.ensure_edit_mode()
        result = self.create_ui_element(name=name, parent=parent, rect_transform=rect_transform)
        if not result["success"]:
        ...
        add_result = self.api.add_component(
            ...
            "LineEdit",
            {
                ...
                "font_size": font_size,
                "color": [255, 255, 255, 255],
                "placeholder_color": [128, 128, 128, 255],
                "alignment": "left",
            },
        )
        return add_result if not add_result["success"] else self.ok("LineEdit created", {"entity": name})

    def create_ui_slider(
        self,
        name: str,
        parent: str,
        value: float = 0.0,
        ...
        Args:
            ...
            step: Step increment.
            horizontal: True for horizontal slider, False for vertical.
            rect_transform: Optional RectTransform overrides.

        Returns:
            ActionResult confirming the Slider entity was created.
        """
        self.ensure_edit_mode()
        result = self.create_ui_element(name=name, parent=parent, rect_transform=rect_transform)
        if not result["success"]:
        ...
        return add_result if not add_result["success"] else self.ok("Slider created", {"entity": name})

    def create_ui_progressbar(
        self,
        name: str,
        parent: str,
        value: float = 0.0,
        ...
        """Create a ProgressBar UI control.
        ...
        Args:
            ...
            min_value: Minimum value.
            max_value: Maximum value.
            horizontal: True for horizontal bar.
            percent_visible: Whether to show percentage text.
            rect_transform: Optional RectTransform overrides.

        Returns:
            ActionResult confirming the ProgressBar entity was created.
        """
        self.ensure_edit_mode()
        result = self.create_ui_element(name=name, parent=parent, rect_transform=rect_transform)
        if not result["success"]:
        ...
        return add_result if not add_result["success"] else self.ok("ProgressBar created", {"entity": name})

    def create_ui_checkbox(
        self,
        name: str,
        parent: str,
        text: str = "",
        ...
        Returns:
            ActionResult confirming the CheckBox entity was created.
        """
        self.ensure_edit_mode()
        result = self.create_ui_element(name=name, parent=parent, rect_transform=rect_transform)
        if not result["success"]:
        ...
        return add_result if not add_result["success"] else self.ok("CheckBox created", {"entity": name})
    ...
    def create_ui_label(
        ...
        text: str = "",
        font_size: int = 16,
        color: Optional[list[int] | tuple[int, int, int, int]] = None,
        alignment: str = "left",
        rect_transform: Optional[Dict[str, Union[str, int, float, bool, list, dict, None]]] = None,
    ) -> ActionResult:
        """Create a Label UI control with rich text support.
        ...
        Args:
            ...
            text: Display text.
            font_size: Font size in pixels.
            color: RGBA color as list or tuple.
            alignment: Text alignment ("left", "center", "right").
            rect_transform: Optional RectTransform overrides.

        Returns:
            ActionResult confirming the Label entity was created.
        """
        self.ensure_edit_mode()
        result = self.create_ui_element(name=name, parent=parent, rect_transform=rect_transform)
        if not result["success"]:
        ...
        add_result = self.api.add_component(
            ...
            "Label",
            {
                ...
                "text": text,
                "font_size": font_size,
                "color": list(color) if color is not None else [255, 255, 255, 255],
                "alignment": alignment,
                "autowrap": False,
                "clip_text": False,
            },
        )
        return add_result if not add_result["success"] else self.ok("Label created", {"entity": name})

    def create_ui_spinbox(
        self,
        name: str,
        parent: str,
        value: float = 0.0,
        ...
        """Create a SpinBox UI control with up/down arrows.
        ...
        Args:
            ...
            prefix: String prepended to displayed value.
            suffix: String appended to displayed value.
            rect_transform: Optional RectTransform overrides.

        Returns:
            ActionResult confirming the SpinBox entity was created.
        """
        self.ensure_edit_mode()
        result = self.create_ui_element(name=name, parent=parent, rect_transform=rect_transform)
        if not result["success"]:
        ...
        return add_result if not add_result["success"] else self.ok("SpinBox created", {"entity": name})

    def create_ui_textedit(
        self,
        name: str,
        parent: str,
        text: str = "",
        ...
        """Create a TextEdit UI control for multi-line text input.
        ...
        Args:
            ...
            max_lines: Maximum line count (0 = unlimited).
            rect_transform: Optional RectTransform overrides.

        Returns:
            ActionResult confirming the TextEdit entity was created.
        """
        self.ensure_edit_mode()
        result = self.create_ui_element(name=name, parent=parent, rect_transform=rect_transform)
        if not result["success"]:
        ...
        return add_result if not add_result["success"] else self.ok("TextEdit created", {"entity": name})

    def set_slider_value(self, entity_name: str, value: float) -> ActionResult:
        """Set the value of a Slider component.

        Args:
            entity_name: Name of the Slider entity.
            value: New slider value.

        Returns:
            ActionResult confirming the value was updated.
        """
        self.ensure_edit_mode()
        return self.api.edit_component(entity_name, "Slider", "value", value)

    def set_progressbar_value(self, entity_name: str, value: float) -> ActionResult:
        """Set the value of a ProgressBar component.

        Args:
            entity_name: Name of the ProgressBar entity.
        ...
        """
        self.ensure_edit_mode()
        return self.api.edit_component(entity_name, "ProgressBar", "value", value)

    def set_lineedit_text(self, entity_name: str, text: str) -> ActionResult:
        """Set the text of a LineEdit component.

        Args:
            entity_name: Name of the LineEdit entity.
            text: New text value.

        Returns:
            ActionResult confirming the text was updated.
        """
        self.ensure_edit_mode()
        return self.api.edit_component(entity_name, "LineEdit", "text", text)
