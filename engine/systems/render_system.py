...
class RenderSystem:
    ...
    def render(self, world: World, override_camera: Optional[rl.Camera2D] = None) -> None:
        """
        Renderiza todas las entidades con Transform.
        
        Args:
            world: El mundo que contiene las entidades
            override_camera: Cámara opcional para renderizar (ej: Editor Camera)
        """
        if override_camera:
            rl.begin_mode_2d(override_camera)
    ...
    def _draw_selection_highlight(self, entity: Entity) -> None:
        """Dibuja un borde resaltado alrededor de la entidad seleccionada."""
        transform = entity.get_component(Transform)
        if not transform:
            return
            
        # Determinar bounds (lógica simplificada similar a SelectionSystem)
        ...
        if sprite:
            if sprite.width > 0: width = sprite.width
                ...
                # Si hay imagen, intentar obtener tamaño real (si está cargada)
                # Esto es una optimización, si no está cargada usamos default
        ...
        # Dibujar bordes y esquinas
        rl.draw_rectangle_lines_ex(
            rl.Rectangle(left, top, width, height),
            2,
            color
        )
