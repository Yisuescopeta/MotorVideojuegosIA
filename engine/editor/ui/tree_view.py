...
class TreeModel:
    ...
    @classmethod
    def build(cls, world: Any) -> "TreeModel":
        ...
        def build_node(entity: Any, depth: int, active_path: set[int]) -> TreeNode:
            entity_id = int(getattr(entity, "id"))
...
def _iter_entities(world: Any) -> list[Any]:
...
def _collect_parent_ids(world: Any, entities: list[Any], entity_by_name: dict[str, Any]) -> dict[int, int | None]:
...
def _collect_parent_ids_from_world_children(get_children: Any) -> dict[int, int | None]:
    parent_by_id: dict[int, int | None] = {}
    pending: list[tuple[int | None, Any]] = [(None, child) for child in list(get_children(None))]
    visited: set[int] = set()
    while pending:
        ...
        for child in list(get_children(getattr(entity, "name", None))):
            pending.append((entity_id, child))
    return parent_by_id


def _parent_id_from_entity(entity: Any, entity_by_name: dict[str, Any]) -> int | None:
...
def _component_type_names(entity: Any) -> list[str]:
    components = []
    ...
    if callable(iter_components):
        ...
        components = list(entity.get_all_components())
    return sorted(type(component).__name__ for component in components)


def _get_component_instance(entity: Any, component_name: str) -> Any | None:
...
def _world_version(world: Any) -> int:
