...
def infer_property_kind(value: Any) -> PropertyKind | None:
...
def build_inspector_model_from_dict(entity_data: dict[str, Any]) -> InspectorModel:
...
def _descriptors_from_mapping(values: dict[str, Any]) -> list[PropertyDescriptor]:
...
def _is_numeric_sequence(value: tuple[Any, ...] | list[Any]) -> bool:
    return len(value) in (2, 3, 4) and all(isinstance(item, (int, float)) and not isinstance(item, bool) for item in value)


def _numeric_sequence_kind(value: tuple[Any, ...] | list[Any]) -> PropertyKind | None:
    if len(value) == 4:
