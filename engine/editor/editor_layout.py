...
class EditorLayout:
    ...
    def update_input(self) -> None:
        ...
        # Refrescar Project Panel (scroll, clicks)
        # Asumimos que bottom_rect es su área
        if self.project_panel:
             # Por ahora hardcodemos update? No, render maneja input UI simple en Raylib inmediato
             # Pero si tiene lógica persistente (como doble click timer), debería ir aquí.
             # ProjectPanel.refresh() se llama on demand, no cada frame.
             pass
             
        mouse_pos = rl.get_mouse_position()
        
        # A. Toolbar / Tabs interaction
        if rl.is_mouse_button_pressed(rl.MOUSE_BUTTON_LEFT):
            if rl.check_collision_point_rec(mouse_pos, self.tab_scene_rect):
                ...
                # Toggle Play/Stop request
                # El estado real se gestiona en Game, aquí solo enviamos señal toggle
                self.request_play = True # Game debe leer esto y resetearlo
    ...
    def get_scene_mouse_pos(self) -> rl.Vector2:
        ...
        # Usamos center_rect que es donde se dibuja la scene
        local_pos = rl.Vector2(
            screen_mouse.x - self.center_rect.x,
            screen_mouse.y - self.center_rect.y
        )
    ...
    def is_mouse_in_scene_view(self) -> bool:
        if self.active_tab != "SCENE":
            return False
        return rl.check_collision_point_rec(rl.get_mouse_position(), self.center_rect)
