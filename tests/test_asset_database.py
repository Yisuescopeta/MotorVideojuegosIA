...
class AssetDatabaseTests(unittest.TestCase):
    ...
    def _write_settings(self, overrides: dict | None = None) -> None:
        """Escribe settings/project_settings.json en el temp dir."""
        ...
        (settings_dir / "project_settings.json").write_text(json.dumps(settings, indent=4), encoding="utf-8")

    def test_detect_asset_type(self) -> None:
        """Detecta tipo de asset por extension."""
        self.assertEqual(self.asset_service.detect_asset_type("image.png"), "textures")
        self.assertEqual(self.asset_service.detect_asset_type("image.jpg"), "textures")
        self.assertEqual(self.asset_service.detect_asset_type("image.jpeg"), "textures")
        self.assertEqual(self.asset_service.detect_asset_type("sound.wav"), "audio")
        self.assertEqual(self.asset_service.detect_asset_type("sound.mp3"), "audio")
        self.assertEqual(self.asset_service.detect_asset_type("sound.ogg"), "audio")
        self.assertEqual(self.asset_service.detect_asset_type("font.ttf"), "fonts")
        self.assertEqual(self.asset_service.detect_asset_type("font.otf"), "fonts")
        self.assertEqual(self.asset_service.detect_asset_type("level.json"), "scene_data")
        self.assertEqual(self.asset_service.detect_asset_type("unknown.xyz"), "unknown")
        self.assertEqual(self.asset_service.detect_asset_type("noextension"), "unknown")

    def test_get_import_options(self) -> None:
        """Lee opciones de import desde project_settings.json."""
        self._write_settings()
        options = self.asset_service.get_import_options("textures")
        self.assertEqual(options["filter_mode"], "linear")
        self.assertEqual(options["generate_mipmaps"], False)
        self.assertEqual(options["compress"], False)

    def test_import_options_textures(self) -> None:
        """Opciones de texturas incluyen filter_mode."""
        self._write_settings()
        options = self.asset_service.get_import_options("textures")
        self.assertIn("filter_mode", options)
        self.assertEqual(options["filter_mode"], "linear")

    def test_import_options_audio(self) -> None:
        """Opciones de audio incluyen sample_rate."""
        self._write_settings()
        options = self.asset_service.get_import_options("audio")
        self.assertIn("sample_rate", options)
        self.assertEqual(options["sample_rate"], 44100)
        self.assertIn("channels", options)
        self.assertEqual(options["channels"], 2)
        self.assertIn("stream", options)
        self.assertEqual(options["stream"], False)
