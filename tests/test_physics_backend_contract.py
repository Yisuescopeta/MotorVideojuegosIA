...
class TestCrossBackendContract:
    ...
    def test_both_backends_prevent_falling_through_floor(self) -> None:
        """Ambos backends deben evitar que el player atraviese el suelo."""
        # Legacy
        w1, p1, g1 = _make_world_with_floor()
        legacy = LegacyAABBPhysicsBackend(physics_system=None, collision_system=None)
        legacy.step(w1, 0.0)
        r1 = legacy.move_and_slide(p1, (0, 300), 1 / 60)

        # Box2D
        w2, p2, g2 = _make_world_with_floor()
        box2d = Box2DPhysicsBackend(gravity=600)
        box2d.sync_world(w2)
        box2d.move_and_slide(p2, (0, 300), 1 / 60)
        box2d.step(w2, 1 / 60)

        # Transform de player actualizado por step() en Box2D
        t2 = p2.get_component(Transform)
        r2_y = t2.y if t2 else 0.0

        ground_top = 200 - 16
        assert r1.position_y + 16 <= ground_top + 2.0, f"Legacy: player atravesó suelo (y={r1.position_y})"
        assert r2_y + 16 <= ground_top + 2.0, f"Box2D: player atravesó suelo (y={r2_y})"
    ...
    def test_both_backends_return_on_floor_flag(self) -> None:
        """Ambos backends deben reportar on_floor al colisionar con suelo."""
        # Legacy
        w1, p1, g1 = _make_world_with_floor()
        legacy = LegacyAABBPhysicsBackend(physics_system=None, collision_system=None)
        legacy.step(w1, 0.0)
        r1 = legacy.move_and_slide(p1, (0, 200), 1 / 60)

        # Box2D
        w2, p2, g2 = _make_world_with_floor()
        box2d = Box2DPhysicsBackend(gravity=600)
        box2d.sync_world(w2)
        box2d.move_and_slide(p2, (0, 200), 1 / 60)
        box2d.step(w2, 1 / 60)
        r2 = box2d.move_and_slide(p2, (0, 0), 1 / 60)

        assert r1.on_floor, "Legacy: debería detectar suelo"
        assert r2.on_floor, "Box2D: debería detectar suelo"
